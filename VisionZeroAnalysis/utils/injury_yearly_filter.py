...
cols_to_keep = ['FID', 'YR', 'NODEID','Lon','Lat','Injuries','PedInjurie','BikeInjuri','MVOInjurie']
...
df1['Fatalities'] = df1['Injuries_COUNT'].apply(lambda x: 1 if x >= 1 else 0)
df1['PedFatalit'] = df1['PedInjurie_COUNT'].apply(lambda x: 1 if x >= 1 else 0)
df1['BikeFatali'] = df1['BikeInjuri_COUNT'].apply(lambda x: 1 if x >= 1 else 0)
df1['MVOFatalit'] = df1['MVOInjurie_COUNT'].apply(lambda x: 1 if x >= 1 else 0)
